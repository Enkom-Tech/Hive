# Hive Plugin System Specification

Status: proposed complete spec for the plugin system (post–first release)

The plugin system is in the **deferred backlog** ([SPEC-implementation.md §20](../SPEC-implementation.md)); this spec is the target when that work is picked up.

This document is the complete specification for Hive's plugin and extension architecture.
It expands the brief plugin notes in [doc/SPEC.md](../SPEC.md) and should be read alongside the design rationale in [EXTENSION_MODEL.md](./EXTENSION_MODEL.md).

This is not part of the implementation contract in [doc/SPEC-implementation.md](../SPEC-implementation.md).
It is the full target architecture for the plugin system that should follow the first release.
...
Hive plugin design is based on the following assumptions:

1. Hive is single-tenant and self-hosted.
2. Plugin installation is global to the instance.
3. "Companies" remain core Hive business objects, but they are not plugin trust boundaries.
4. Board governance, approval gates, budget hard-stops, and core task invariants remain owned by Hive core.
...
### 2.1 Alignment with design rationale

This spec implements the architecture recommended in [EXTENSION_MODEL.md](./EXTENSION_MODEL.md).

- **Trust and runtime:** Platform modules are high-trust, in-process; plugins are out-of-process and capability-gated; third-party code does not run in the same trust domain as core by default.
- **Core invariants:** Approvals, budgets, issue state machines, and audit logging are core-owned; plugins observe and extend via typed APIs and events but do not override or bypass these.
- **Install and activation:** Installation and activation are explicit operator actions; no automatic load-from-project or install-and-execute from config alone.
- **Collisions:** Extension points use namespacing and explicit capability rules; no implicit override-by-name of core routes or auth.
...
2. Let plugins add major capabilities without editing Hive core.
...
4. Depend on project-local plugin folders such as `.hive/plugins`.
...
The single Hive deployment an operator installs and controls. For deploying that instance on k3s (control plane, workers, LLM stack), see [K3S-LLM-DEPLOYMENT.md](../K3S-LLM-DEPLOYMENT.md).

### 5.2 Company

A first-class Hive business object inside the instance.
...
A trusted in-process extension loaded directly by Hive core.
...
An installable instance-wide extension package loaded through the Hive plugin runtime.
...
Hive has two extension classes.
...
Hive already has a concrete workspace model:
...
Plugins live under the Hive instance directory.

Suggested layout (Unix: `~/.hive/instances/default/...`; Windows: `%USERPROFILE%\.hive\instances\default\...`):
...
Hive should add CLI commands:

- `pnpm hive plugin list`
- `pnpm hive plugin install <package[@version]>`
- `pnpm hive plugin uninstall <plugin-id>`
- `pnpm hive plugin upgrade <plugin-id> [version]`
- `pnpm hive plugin doctor <plugin-id>`
...
- UI slot IDs are automatically namespaced by plugin ID (e.g. `@hive/plugin-linear:sync-health-widget`), so cross-plugin collisions are structurally impossible
...
Plugins may contribute tools that Hive agents can use during runs.
...
- Hive server starts one worker process per installed plugin
...
Receives one typed Hive domain event.
...
export function definePlugin(definition: PluginDefinition): HivePlugin;

/** Re-exported from Zod for config schema definitions */
export { z } from "zod";

export interface PluginContext {
  manifest: HivePluginManifest;
  config: {
...
```ts
ctx.events.on("plugin.@hive/plugin-git.push-detected", async (event) => {
...
```tsx
// dist/ui/index.tsx
import { usePluginData, usePluginAction, MetricCard, StatusBadge } from "@hive/plugin-sdk/ui";
...
- The host provides **design tokens and shared components** via `@hive/plugin-sdk/ui` so plugins can match the host's visual language without being forced to.
...
### 19.0.1 Plugin UI SDK (`@hive/plugin-sdk/ui`)
...
- Plugin bundles must not import from host internals. They may only import from `@hive/plugin-sdk/ui` and their own dependencies.
...
## 19.6 Shared Components In `@hive/plugin-sdk/ui`
...
The `@hive/plugin-sdk/ui` subpath should also export an `ErrorBoundary` component that plugin authors can use to catch rendering errors without crashing the host page.
...
- secret ref fields: any schema property annotated with `"format": "secret-ref"` renders as a secret picker that resolves through the Hive secret provider system rather than a plain text input
...
1. Core Hive data stays in first-party tables.
2. Most plugin-owned data starts in generic extension tables.
3. Plugin data should scope to existing Hive objects before new tables are introduced.
4. Arbitrary third-party schema migrations are out of scope for the first plugin system.

## 21.2 Core Table Reuse

If data becomes part of the actual Hive product model, it should become a first-party table.

Examples:

- `project_workspaces` is already first-party
- if Hive later decides git state is core product data, it should become a first-party table too
...
2. Secret refs resolve through the existing Hive secret provider system.
...
6. The operator may force-purge immediately via CLI: `pnpm hive plugin purge <plugin-id>`.
...
Plugin install, uninstall, upgrade, and config changes **must** take effect without restarting the Hive server. This is a normative requirement, not optional.
...
```ts
import { createTestHarness } from "@hive/plugin-test-harness";
...
For developing a plugin against a running Hive instance:

- The operator installs the plugin from a local path: `pnpm hive plugin install ./path/to/plugin`
...
The host should publish a starter template (`create-hive-plugin`) that scaffolds:

- `package.json` with correct `hivePlugin` keys
...
- `@hive/plugin-workspace-files`
- `@hive/plugin-terminal`
- `@hive/plugin-git`
- `@hive/plugin-linear`
- `@hive/plugin-github-issues`
- `@hive/plugin-grafana`
- `@hive/plugin-runtime-processes`
- `@hive/plugin-stripe`
...
- `@hive/plugin-sdk` — the complete plugin SDK

The package uses subpath exports to separate worker and UI concerns:

- `@hive/plugin-sdk` — worker-side SDK (context, events, state, tools, logger, `definePlugin`, `z`)
- `@hive/plugin-sdk/ui` — frontend SDK (bridge hooks, shared components, design tokens)
...
2. **Tied to API version**: Each major SDK version corresponds to exactly one plugin `apiVersion`. When `@hive/plugin-sdk@2.x` ships, it targets `apiVersion: 2`. Plugins built with SDK 1.x continue to declare `apiVersion: 1`.
...
1. Plugin author updates `@hive/plugin-sdk` dependency.
...
- webhooks
- settings page
- plugin UI bundle loading, host bridge, and `@hive/plugin-sdk/ui`
...
- `@hive/plugin-test-harness`
- `create-hive-plugin` starter template
...
Hive should not implement a generic in-process hook bag modeled directly after local coding tools.

Hive should implement:
...
That is the complete target design for the Hive plugin system.
