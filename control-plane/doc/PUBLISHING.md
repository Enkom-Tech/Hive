...
Low-level reference for how Hive packages are built for npm.
...
The CLI package imports code from workspace packages such as:
...
- `npx hive@canary onboard` can install them explicitly
- `pnpm hive onboard` continues to resolve `latest`
