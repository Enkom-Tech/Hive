...
`PAP-447` asks how Hive should support worktree-driven coding workflows for local coding agents without turning that into a universal product requirement.
...
- the workflow should reuse the same Hive instance and embedded database instead of creating a blank environment
- local agent auth should remain low-friction

At the same time, we do not want to hard-code "every agent uses git worktrees" into Hive:

- some operators use Hive to manage Hive and want worktrees heavily
- other operators will not want worktrees at all
- not every adapter runs in a local git repository
- not every adapter runs on the same machine as Hive
- Claude and Codex expose different built-in affordances, so Hive should not overfit to one tool

## Core Product Decision

Hive should model **execution workspaces**, not **worktrees**.
...
- local adapters may receive a real filesystem cwd produced by Hive
...
- Hive should not assume that every adapter can see or use a host filesystem path directly
...
If Hive makes worktrees agent-first, it will blur:
...
- Hive core resolves and records execution workspace state
...
That gives Hive room to support:
...
- Hive handles workspace runtime services under the hood using project/workspace policy
...
Once Hive is creating isolated issue checkouts, it is implicitly touching a bigger workflow:
...
- only add adapter-specific runtime UI when an adapter truly needs settings that Hive cannot infer
...
In the target model, the **worker** (drone) spawns the agent in the workspace; config and execute flow are in `infra/worker`. UI that previously referenced adapter-specific config fields (e.g. codex-local, claude-local) should be updated to worker/run config or removed in favor of worker-driven workspace policy.

Technical direction:

- workspace policy is resolved by the control plane and passed to the worker in the run message over WebSocket
- worker sets execution cwd/workspace per run; project execution workspace policy is inherited unless overridden

## Worker and Orchestration Changes

### 10. Worker (drone) behavior

The worker spawns the agent in the workspace; config and execute flow live in `infra/worker`. No per-adapter packages in the control plane.

#### Control plane vs drone: where worktrees live

For k3s deployment and worker workspace layout see [K3S-LLM-DEPLOYMENT.md](../K3S-LLM-DEPLOYMENT.md).

Worktree **realization** happens on the **control plane** (or wherever the project repo lives): the control plane creates the worktree (e.g. under `.hive/worktrees/<branch>`), runs the provision command, and obtains `cwd` / `worktreePath`. The control plane injects **context.hiveWorkspace** (`cwd`, `worktreePath`, etc.) into the run context sent to the worker. When the agent runs on the **same machine** as the control plane (e.g. control-plane-local adapter), that path is valid and file access to the worktree is effectively automatic. When the agent runs on a **remote drone**, the run message does not currently supply a workspace path used by the drone for process cwd; the drone uses `HIVE_WORKSPACE` or default. So **file access to the control-plane worktree is not automatic** for remote drones. A future extension would be to pass `workspaceDir` or `workspacePath` in the run payload and have the drone set the agent's cwd when the path is available (e.g. shared storage or drone-side worktree creation).

Tasks:

- continue to accept resolved workspace/runtime context from heartbeat (run/cancel over worker's WebSocket link)
- stop assuming the agent config is the primary source of workspace policy
- worker applies workspace and env per run
...
- when issue moves to done, should Hive auto-commit?
- should it auto-push?
...
Hive already has the right foundation for a project-first model.
...
Session continuity is cwd-bound where the worker runs the agent. The worker executes the agent process (e.g. in `infra/worker`); the clean insertion point is before execution: resolve the final execution cwd first, then let the worker run the agent normally.

### Worker credentials

The worker holds credentials and injects env/context when spawning the agent. Auth is worker credential–based (see DRONE-SPEC and MANAGED-WORKER-ARCHITECTURE); agents do not receive API keys. Manual-local bootstrap is a related auth ergonomics concern, not a reason to make worktrees a core invariant.
...
- Codex CLI does not present a single first-class portable CLI worktree abstraction that Hive should mirror directly

Implication:

- for `codex_local`, Hive should usually create/select the checkout itself and then launch Codex inside that cwd
...
- but Hive should still treat that as an adapter optimization, not the canonical cross-adapter model
...
Because of that, Hive should separate:
...
Hive should be able to express intentions such as:
...
These are not specific to Hive. They are a common property of working in a dev workspace, whether local or remote.
...
```json
{
  "workspaceStrategy": {
    ...
    "worktreeParentDir": ".hive/instances/default/worktrees/projects/{{project.id}}",
...
Do not model this as a Hive-specific `devServer` flag.
...
Hive should distinguish between:
...
Hive should normalize the reported metadata without requiring every adapter to look like a host-local process.
...
In addition to the local git helper, Hive should define a generic runtime service manager contract.
...
- remote sandbox adapter: create its own isolated workspace from repo/ref/branch intent and report the realized remote workspace metadata back to Hive
...
For runtime services specifically, Hive should eventually track normalized fields such as:
...
- runtime services are expressed generically, not as Hive-only dev-server flags
...
3. Inject resolved execution workspace details into `context.hiveWorkspace` for local adapters and into a generic execution-workspace intent payload for adapters that need structured remote realization.
...
- same embedded DB / Hive instance can be reused through env/config injection when appropriate
...
- Hive can decide whether to reuse or start a fresh service deterministically
...
3. Test against Hive-on-Hive workflows first.
...
7. Workspace runtime services are modeled generically, not as Hive-specific dev-server toggles.
