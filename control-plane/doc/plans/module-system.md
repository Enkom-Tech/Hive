...
Hive's module system lets you extend the control plane with new capabilities — revenue tracking, observability, notifications, dashboards — without forking core. Modules are self-contained packages that register routes, UI pages, database tables, and lifecycle hooks.
...
|---------|-----------|----------------|
| **Module** | A package that extends Hive's API, UI, and data model | Yes |
...
modules/
  observability/
    hive.module.json     # manifest (required)
    src/
...
- **`id`**: Unique identifier, used as the npm package name suffix (`@hive/mod-observability`)
...
```typescript
import type { ModuleAPI } from "@hive/core";
...
```
1. Scan modules/ directory for hive.module.json manifests
...
export const modulePages = [
  {
    path: "/observability",
    label: "Observability",
    component: lazy(() => import("@hive/mod-observability/ui")),
  },
];
...
export const dashboardWidgets = [
  {
    ...
    label: "Token Burn Rate",
    placement: "dashboard",
    component: lazy(() => import("@hive/mod-observability/ui").then(m => ({ default: m.TokenBurnRateWidget }))),
  },
];
```
...
The Company Store is a registry for discovering and installing modules and templates. Initially it is a curated GitHub repo with a JSON index. Later it could become a hosted service.
  ...
  "modules": [
    {
      ...
      "repo": "github:hive/mod-observability",
      "version": "0.1.0",
  ...
  "templates": [
    {
      ...
      "url": "https://store (Hive domain)/templates/startup-in-a-box.json",
      "tags": ["startup", "team"]
    }
  ]
}
...
```bash
pnpm hive store list                    # browse available modules and templates
pnpm hive store install <module-id>     # install a module
pnpm hive store import <template-id>    # import a company template
pnpm hive store export                  # export current company as template
```
...
| **Multi-tenant** | Separate companies/orgs within one Hive instance | `server:started` |
...
4. **Module config** — `hive.config.json` with per-module config, env var interpolation
...
10. **`@hive/module-sdk`** — TypeScript types for `ModuleAPI`, `HookEvent`, `HookHandler`, manifest schema
