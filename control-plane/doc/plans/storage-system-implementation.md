...
Add a single storage subsystem for Hive that supports:
...
- API serves attachment bytes through authenticated Hive endpoints.
...
- Default local path is under instance root: `~/.hive/instances/<instanceId>/data/storage`.
...
- `hive onboard` writes a valid `storage` config block by default.
- `hive configure --section storage` can switch between local and s3 modes.
...
- Enforce max size and image/content-type allowlist in the initial implementation.
...
- `hive doctor` reports actionable storage status.
