...
Keep Hive low-friction while making the mode model simpler and safer:
...
- Runtime deployment modes are currently `local_trusted | authenticated` (`packages/shared/src/constants.ts`).
...
- `authenticated` uses Better Auth sessions and `authUsers` rows (`server/src/auth/better-auth.ts`, `packages/db/src/schema/auth.ts`).

## Bootstrap/Admin

- `authenticated` requires `BETTER_AUTH_SECRET` and reports bootstrap status from `instance_user_roles` (`server/src/index.ts`, `server/src/routes/health.ts`).
...
1. `pnpm hive onboard` is interactive-first and defaults to `local_trusted`.
2. authenticated mode is one runtime mode with `private/public` exposure guidance.
3. `pnpm hive doctor` works flagless with mode-aware checks.
...
For production k3s deployment (control plane, workers, vLLM, model gateway), see [K3S-LLM-DEPLOYMENT.md](../K3S-LLM-DEPLOYMENT.md).
