# CEO Agent Creation and Hiring Governance Plan
...
## 5.6 Reflection endpoints for agent configuration

In the target model, agent configuration is **worker-driven**; there is no per-adapterType LLM config endpoint. The control plane does not serve adapter-specific config docs. Configuration and execution are defined by the drone (see [DRONE-SPEC.md](../DRONE-SPEC.md)). A generic `GET /llms/agent-configuration.txt` may still list company/agent context and "how to hire" API sequence links for board/CEO use; per-adapter reflection is out of scope.

Auth for any config-read endpoints: same gate as board/CEO/`can_create_agents`.

## 6. Worker/Drone configuration

Agent configuration and execution are implemented in the worker (drone). See [DRONE-SPEC.md](../DRONE-SPEC.md) and `infra/worker` for env, run/cancel/status/log over WebSocket, and agent lifecycle. For deploying the worker on k3s see [K3S-LLM-DEPLOYMENT.md](../K3S-LLM-DEPLOYMENT.md). No ServerAdapterModule or per-adapter packages in the control plane; the drone spawns agents and applies identity/permissions per run.
...
## 8. New Skill: `hive-create-agent`

Create new skill directory:

- `skills/hive-create-agent/SKILL.md`
- `skills/hive-create-agent/references/api-reference.md`

Skill responsibilities:

- Discover agent/company context and hire API (worker-driven config; see DRONE-SPEC)
...
Also update `skills/hive/SKILL.md` to reference this skill for hiring workflows.
...
- optional generic config-read endpoint (company/agent context, hire API links); no per-adapterType routes (target is worker-driven; see DRONE-SPEC)
...
- `hive-create-agent` skill + docs updates
