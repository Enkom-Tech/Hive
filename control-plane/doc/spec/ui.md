...
Hive’s UI is a professional control plane for continuous, all-day use in operational environments. It is fast, information-dense, and scannable; clarity and restraint override decoration, and every visible element must earn its place. Theme follows system preference (light/dark), and a cohesive design token system keeps layout, spacing, and color semantics consistent across surfaces. Primary screens show the right information at the right moment, with progressive disclosure and drill-downs for detail; complex, multi-state views are scannable at a glance, with clear prioritization of critical status, actionable items, and anomalies.
...
- **Theme follows system by default.** Default theme respects `prefers-color-scheme` (light or dark). User can set Light, Dark, or System in settings; saved preference overrides system. Neutral grays (not pure black in dark); accent colors used sparingly for status and priority. Text is the primary visual element.
- **Accessible by default.** Focus visible, semantics correct, contrast sufficient. Support reduced motion; don't rely on animation for meaning.
- **Reversible or confirmed.** Destructive actions (delete, reject) use a confirmation step or a short undo window so mistakes are recoverable.

### Color System

Hive uses a shared TRON-style glassmorphism language with a single **product accent** token (`--accent`) used only for borders, glows, active states, key metrics, and primary actions. The glass and background stay neutral (no color tint in blur or fill).

- **Background (dark):** `#020617` (slate-950). In dark mode a fixed page layer (`.page-bg`) provides this base plus subtle grayscale grid and noise so glass panels have texture to blur.
- **Surface/Card:** Semantic `--card`; glass variant uses `bg-white/5`–`bg-white/10`, `border-white/10`, `backdrop-blur-xl` with 0 or subtle radius.
- **Border (neutral):** `border-white/10` for glass; semantic `--border` where appropriate.
- **Text primary / secondary:** `--foreground`, `--muted-foreground`.
- **Accent (product):** Set via `data-theme` on `<html>`. Default Hive: `--accent: #f59e0b` (amber-500), `--accent-rgb: 245 158 11`. My Grid: `--accent: #00ffff`, `--accent-rgb: 0 255 255`. Use for: breadcrumb bar bottom line, sidebar active left border, metric card left bar and value, primary/secondary glass buttons (border + glow), focus ring, status chips. Do not tint glass panels with accent.

**Theme switching:** Set `data-theme="hive"` or `data-theme="my-grid"` on `<html>` (e.g. in `index.html` or via ThemeContext). All components that depend on the product accent read `var(--accent)` or Tailwind `--color-accent`; changing the attribute swaps the accent without structural changes. Usability: prefer readable contrast on glass, softer glows in dense areas, optional subtle radius (`rounded-md`), and respect `prefers-reduced-motion`.
...
- **Focus ring:** Use `--accent` (2px ring, offset 2px). Must be visible on all interactive elements.
- **Disabled:** text and icons at 40% opacity; cursor not-allowed; no hover/active change.
- **Destructive:** `hsl(0, 70%, 55%)` for delete/reject actions. Use sparingly.

Text on status-colored backgrounds (e.g. badges) must meet contrast: use white or near-black depending on background luminance.

Light theme uses the same semantic roles (background, surface, border, text primary/secondary, accent, status, focus, disabled, destructive) with appropriate luminance; status and priority colors keep the same meaning across themes.

### Typography

- **Font:** System font stack: `-apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, sans-serif`. Optionally load Inter for a more branded look; do not rely on it for layout.
- **Body:** 13px / 1.5 line-height. Max line length for long-form content (e.g. description, comments): 65ch.
- **Labels/metadata:** 11px / uppercase tracking
- **Headings:** 14-18px / semi-bold, never all-caps

### Icons

Use `lucide-react` throughout. Every sidebar item, every status indicator, every action button should have an icon. Icons are 16px in nav, 14px inline. Icon-only buttons: minimum 32×32px hit area (44×44px on touch targets when responsive).

### Spacing and touch targets

- **Base unit:** 4px. Spacing scale: 4, 8, 12, 16, 24, 32.
- **List row min-height:** 40px so that tap/click and focus ring are comfortable.
- **Touch:** At breakpoints where the UI is used with touch, interactive elements must have at least 44×44px touch target (padding can extend hit area beyond visible bounds).

### Accessibility

- **Focus order:** Matches visual order. Modals and panels trap focus; on close, focus returns to the trigger.
- **Focus visibility:** Every focusable element shows the focus ring; no `outline: none` without a visible replacement.
- **Live regions:** Use `aria-live="polite"` for toasts and non-urgent updates (e.g. "Issue updated"). Use `aria-live="assertive"` only for critical errors that need immediate announcement.
- **Labels:** All form controls and icon-only actions have visible or `aria-label`/`aria-labelledby`. Status and priority controls expose current value to assistive tech.
- **Reduced motion:** Respect `prefers-reduced-motion: reduce`. Where motion is used, provide an instant fallback (no or minimal duration). Do not use motion to convey critical information alone.

### Motion and transitions

- **Duration:** 150ms for micro feedback (hover, focus, checkbox); 200ms for panel open/close and sheet slide; 250ms for route/content transitions if used.
- **Easing:** `ease-out` for enter, `ease-in` for exit; or use a single `ease-in-out` for small UI changes.
- **Scope:** Use for panel open/close, sidebar collapse, modal appear, list reorder (Kanban), and optional page transitions. Do not animate loading skeletons in a way that distracts; a gentle opacity or shimmer is enough.
- **Reduced motion:** When `prefers-reduced-motion: reduce` is set, use 0ms or under 50ms and avoid parallax or decorative motion.
...
User can close the properties panel on detail views; this preference is persisted (e.g. localStorage). When preference is "closed", detail views open with the panel closed by default; the panel toggle in the breadcrumb bar remains available.
...
- When the sidebar is collapsed to icons, each icon shows a tooltip (title) on hover/focus with the full label so users don't have to expand to identify the item.
...
**Breadcrumb overflow:** When the path has many segments, collapse the left side: show first segment, ellipsis, then the last one or two segments (e.g. `Projects › … › CLIP-42`). Keep the current entity and its actions visible; earlier segments remain available via the ellipsis dropdown or by clicking the first segment.
...
1. **Checkbox** — for bulk selection. Always visible; muted style by default (e.g. faint border, no fill), stronger on row hover or when the row has focus. On touch viewports, show at full emphasis (left of priority).
...
- Click checkbox → selects for bulk actions. When any checkbox is selected, a bulk action bar appears at the top of the list (see below).
- Hover → checkbox and row get subtle background highlight.
- Right-click → context menu (same actions as three-dot menu).

**Bulk action bar:**
When one or more issues are selected, a sticky bar appears at the top of the issue list (below the status tabs and filter bar). Same actions; bar stays in view when scrolling and does not cover list content.
```
┌─────────────────────────────────────────────────────────┐
│  3 selected    [Status ▼] [Priority ▼] [Assignee ▼] [Project ▼]  [🗑 Delete]  [✕ Cancel] │
└─────────────────────────────────────────────────────────┘
```
Delete: confirmation dialog (e.g. "Delete N issues? This can't be undone.") with [Cancel] and [Delete]. See Section 1 (Reversible or confirmed) for consistency.
...
### 5.4 New Issue (Modal or Full Page)

Triggered by the sidebar pencil icon, keyboard shortcut `C`, or the `+` buttons in the issue list.

**When to use modal vs full page:**
- **From any issue list context** (main Issues list, or project detail Issues tab): open as a **full-page** view by default. Breadcrumb e.g. "Issues › New issue". Back or cancel returns to the list. Same content and fields as the modal.
- **From sidebar** (pencil icon) or **global shortcut** (C, Cmd+K → Create issue): open as a **modal** so the current view (e.g. list, dashboard) stays visible and the user can dismiss quickly.
When opened as a modal, the expand icon opens the same form in a full page (e.g. for long descriptions).
...
- Expand icon (when in modal: open as full page). Close `×`.
...
**Footer:**
- Attachment button (hexagon icon).
...
├─────────────────────────────────────────────────────────┤
│ [icon] Hive Auth     Backlog     CTO     Feb 20   │
...
- Invocation: managed worker (worker / run config summary)
- Current task (if any)
...
**Right pane properties:** Status, Role, Title, Reports To, Invocation (Worker), Context Mode, Budget (monthly), Spent (monthly), Last Heartbeat.
...
- Inline **[Approve]** and **[Reject]** buttons for simple approvals (hire_agent). **Reject:** confirmation step with a **required** reason (short text or selected reason). **Approve:** optional decision note.
...
- Inbox is computed from live data (pending approvals query + alert conditions), not a separate notification table. This keeps the initial implementation simple.
...
├─────────────────────────────────────────────────────────┤
│ Jump to                                                 │
│   📊 Dashboard    📥 Inbox    📋 Issues    📁 Projects  │
│   🤖 Agents       💰 Costs                              │
├─────────────────────────────────────────────────────────┤
│ Recent                                                  │
...
- **When input is empty:** show (1) **Jump to** — Dashboard, Inbox, Issues, Projects, Agents, Costs (icon + label, Enter to navigate); (2) **Recent** — recent issues/agents/projects; (3) **Actions** — create issue/agent/project.
...
| `Up` / `Down` | Navigate list selection (same as J/K) |
...
**Shortcut discoverability:** A `?` (or `Shift+/`) shortcut opens a compact overlay listing all shortcuts. No shortcut is the only way to perform an action; every shortcut has a mouse path.

Backspace goes back only when focus is not in an input, textarea, or editable region (e.g. description, comment field); in those, Backspace edits text.
...
- **<1024px:** Sidebar hidden (hamburger menu). Properties panel hidden (toggle or tab). Apply touch targets (min 44×44px) for primary actions.

The properties panel is always dismissible — it should never block the main content. On narrow viewports, list views (issues, agents, projects) may switch to card-style rows or horizontal scroll for tables; avoid horizontal scroll of the entire page.
...
Empty states should use a muted illustration (simple line art, not cartoons) and a single call-to-action. When a filter or search returns no results, show a distinct state: e.g. "No issues match your filters" with an option to clear filters or adjust the query, not the same illustration as "no issues exist."

---

## 19. Loading and Error States

- **Loading:** Skeleton placeholders matching the layout of the expected content (not spinners). Skeleton blocks animate with a subtle shimmer (respect reduced motion).
- **Partial load:** When some data is ready and other is still loading (e.g. list loaded, sidebar counts pending), show the available content and indicate loading only where it applies (e.g. badge skeleton). Do not block the whole view.
- **Error:** Inline error message with a retry button. Retry scope is clear: "Retry" reloads the failed request or view, not the entire app. Never a full-page error unless the app itself is broken.
...
```
/                           → redirect: if unresolved inbox count > 0, go to /inbox; else /dashboard. Optional: redirect to last-visited main route (e.g. /inbox, /issues) when stored in localStorage.
...
### Phase 5: Polish
21. Keyboard shortcuts and shortcut discoverability (`?`)
22. Responsive behavior and touch targets
23. Empty states (including "no results" for filters/search) and loading skeletons
24. Error handling, toasts, and partial-load behavior
25. Saved views (custom filters)
26. Accessibility: focus management, ARIA, reduced motion, contrast check
