...
Maintainer runbook for shipping a full Hive release across npm, GitHub, and the website-facing changelog surface.
...
Every Hive release has four separate surfaces:

1. **Verification** — the exact git SHA must pass typecheck, tests, and build.
2. **npm** — Hive and the public workspace packages are published.
...
VERSION=0.2.8
claude --print --output-format stream-json --verbose --dangerously-skip-permissions --model claude-opus-4-6 "Use the release-changelog skill to draft or update releases/v${VERSION}.md for Hive. Read doc/RELEASING.md and .agents/skills/release-changelog/SKILL.md, then generate the stable changelog for v${VERSION} from commits since the last stable tag. Do not create a canary changelog."
...
HIVE_VERSION=canary ./scripts/docker-onboard-smoke.sh
...
```bash
HOST_PORT=3232 DATA_DIR=./data/release-smoke-canary HIVE_VERSION=canary ./scripts/docker-onboard-smoke.sh
HOST_PORT=3233 DATA_DIR=./data/release-smoke-stable HIVE_VERSION=latest ./scripts/docker-onboard-smoke.sh
```

This is the best existing fit when you want:

- a standalone Hive data dir
- a dedicated host port
- an end-to-end `npx hive ... onboard` check
...
HIVE_PORT=3234 ./scripts/clean-onboard-ref.sh
...
- [ ] You are comfortable with users installing it via `npx hive onboard`
...
- [ ] `npm view hive@latest version` matches the new stable version
...
```bash
VERSION=X.Y.Z
claude -p "Use the release-changelog skill to draft or update releases/v${VERSION}.md for Hive. Read doc/RELEASING.md and .agents/skills/release-changelog/SKILL.md, then generate the stable changelog for v${VERSION} from commits since the last stable tag. Do not create a canary changelog."
```
...
- users need `npx hive onboard` to recover quickly
...
3. verify `npm view hive@latest version`
