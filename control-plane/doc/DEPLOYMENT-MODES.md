...
Hive supports two runtime modes:
...
When running `authenticated` mode, if the only instance admin is `local-board`, Hive emits a startup warning with a one-time high-entropy claim URL.
...
- Implementation contract: `doc/SPEC-implementation.md`
- operator workflows: `doc/DEVELOPING.md` and `doc/CLI.md`
- Humans and permissions: `doc/plans/humans-and-permissions.md`
