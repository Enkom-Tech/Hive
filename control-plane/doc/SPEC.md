...
Hive is a control plane for agentic companies. Target specification for the Hive control plane. Living document — updated incrementally during spec interviews.

---

## 1. Company Model [DRAFT]

A Company is a first-order object. One Hive instance runs multiple Companies. A Company does not have a standalone "goal" field — its direction is defined by its set of Initiatives (see Task Hierarchy Mapping).
...
**Single human Board.** One human operator.
...
Concepts like SOUL.md (identity/mission) and HEARTBEAT.md (loop definition) are **not part of the Hive protocol**. They are adapter-specific configurations. For example, an OpenClaw adapter might use SOUL.md and HEARTBEAT.md files. A Claude Code adapter might use CLAUDE.md. A bare Python script might use command-line args.

Hive doesn't prescribe how an agent defines its identity or behavior. It provides the control plane; the adapter defines the agent's inner workings.

### Agent Configuration [DRAFT]

Agents are invoked and managed via the **managed worker**. The control plane talks only to the worker; the worker spawns and controls the actual agent process. Agent identity and config are logical (name, role, permissions); the worker assigns identity and access per run. See [doc/MANAGED-WORKER-ARCHITECTURE.md](MANAGED-WORKER-ARCHITECTURE.md).

#### Hive Protocol (What Hive Knows)

At the protocol level, Hive tracks:

- Agent identity (id, name, role, title)
- Org position (who they report to, who reports to them)
- Worker assignment (invocation is via worker only)
...
- **Fat payload** — Hive bundles relevant context (current tasks, messages, company state, metrics) into the heartbeat invocation. Suited for simple/stateless agents that can't call back to Hive.
- **Thin ping** — Heartbeat is just a wake-up signal. Agent calls Hive's API to fetch whatever context it needs. Suited for sophisticated agents that manage their own state.

#### Minimum Contract

The minimum requirement to be a Hive agent: **be callable.** That's it. Hive can invoke you via command or webhook. No requirement to report back — Hive infers basic status from process liveness when it can.

#### Integration Levels

Beyond the minimum, Hive provides progressively richer integration:

1. **Callable** (minimum) — Hive can start you. That's the only contract.
...
Hive ships **default agents** that demonstrate full integration: progress tracking, cost instrumentation, and a **Hive skill** (e.g. SKILL.md for interacting with the Hive API) for task management. These serve as both useful defaults and reference implementations for adapter authors.
...
Agents can create tasks and assign them to agents outside their reporting line. This is the mechanism for cross-team collaboration. These rules are primarily encoded in the Hive SKILL.md which is recommended for all agents. Hive enforces the tooling and some light governance, but the cross-team rules below are mainly implemented by agent decisions.
...
The heartbeat is a protocol, not a runtime. Hive defines how to initiate an agent's cycle. What the agent does with that cycle — how long it runs, whether it's task-scoped or continuous — is entirely up to the agent.

### Execution: Managed Worker

Invocation is via the **managed worker**. The control plane sends run/stop/status to the worker; the worker spawns and controls agents and reports back. See [doc/MANAGED-WORKER-ARCHITECTURE.md](MANAGED-WORKER-ARCHITECTURE.md).

### Control Plane – Worker Contract

At the worker boundary, the control plane:

- **Run** — control plane sends a run request to the worker over the worker's WebSocket link; worker spawns the agent and sends status (and logs) over the same link.
- **Status** — worker sends run status and log stream over the WebSocket.
- **Cancel** — control plane signals the worker to stop a run over the WebSocket; worker performs graceful termination then force kill if needed.

Cost reporting and task updates flow through the Hive REST API (worker calls on behalf of agents).

### What Hive Controls

- **When** to fire the heartbeat (schedule/frequency, per-agent)
- **How** to fire it (control plane requests run from worker)
- **What context** to include (thin ping vs. fat payload, per-agent)

### What Hive Does NOT Control
...
- Health monitoring — how does Hive distinguish "stuck" from "working on a long task"?
...
Token/LLM cost budgeting is a core part of Hive. External revenue and expense tracking is a future plugin.

### Cost Reporting

Fully-instrumented Agents report token/API usage back to Hive. Costs are tracked at every level:
...
Hive ships default Agent templates:

- **Default Agent** — a basic Claude Code or Codex loop. Knows the **Hive skill** (SKILL.md) so it can interact with the task system, read Company context, report status.
...
A skill definition that teaches agents how to interact with Hive. Provides:
...
The key constraint: it must be trivial to go from "I'm trying this on my machine" to "my agents are running on remote servers talking to my Hive instance."

#### Agent Authentication

The **worker** holds credentials and authenticates to the control plane. Agents do not receive or use API keys; they interact with the control plane only via the worker (e.g. tools/MCP). See [doc/MANAGED-WORKER-ARCHITECTURE.md](MANAGED-WORKER-ARCHITECTURE.md).
...
When a human completes a task, the control plane can notify the worker over the worker's WebSocket link (e.g. run or work-available message) so the requesting agent can be woken. This keeps humans rare but possible participants in the workflow.

The agents are discouraged from assigning tasks to humans in the Hive SKILL, but sometimes it's unavoidable.

### API Design

**Single unified REST API.** The same API serves both the frontend UI and agents. Authentication determines permissions — board auth has full access, worker credentials have scoped access (workers act on behalf of agents).

No separate "agent API" vs. "board API." Same endpoints, different authorization levels.

### Work Artifacts

Hive does **not** manage work artifacts (code repos, file systems, deployments, documents). That's entirely the agent's domain. Hive tracks tasks and costs. Where and how work gets done is outside scope.

### Open Questions

- Real-time updates to the UI — WebSocket? SSE? Polling? (Worker–control plane uses WebSocket only; see MANAGED-WORKER-ARCHITECTURE.)
- Worker credential scoping — what exactly can a worker access on behalf of agents? Only their own tasks? Their team's? The whole Company?

### Crash Recovery: Manual, Not Automatic

When an agent crashes or disappears mid-task, Hive does **not** auto-reassign or auto-release the task. Instead:

- Hive surfaces stale tasks (tasks in `in_progress` with no recent activity) through dashboards and reporting
- Hive does not fail silently — the auditing and visibility tools make problems obvious
...
**Principle: Hive reports problems, it doesn't silently fix them.** Automatic recovery hides failures. Good visibility lets the right entity (human or agent) decide what to do.

### Plugin / Extension Architecture

The core Hive system must be extensible. Features like knowledge bases and external revenue tracking should be addable as **plugins** without modifying core. This means:

- Well-defined API boundaries that plugins can hook into
- Event system or hooks for reacting to task/Agent lifecycle events
- Target is a single adapter (managed worker); no multi-adapter plugin story. Core remains extensible for other plugins (knowledge base, revenue tracking).
- Plugin-registrable UI components (future)

This is not in the initial deliverable (we're not building a plugin framework upfront), but the architecture should not paint us into a corner. Keep boundaries clean so extensions are possible. Target plugin architecture: [doc/plugins/PLUGIN_SPEC.md](plugins/PLUGIN_SPEC.md) (deferred).
...
- [ ] **Agent CRUD** — create/edit/pause/resume Agents (worker assignment / config)
- [ ] **Org chart** — define reporting structure, visualize it
- [ ] **Managed worker adapter** — control plane talks to worker (run/stop/status); worker spawns and controls agents; worker-held credentials
...
- [ ] **Default agent** — basic Claude Code/Codex loop with Hive skill
- [ ] **Default CEO** — strategic planning, delegation, board communication
- [ ] **Hive skill (SKILL.md)** — teaches agents to interact with the API
...
**Anti-goal for core.** The knowledge base is not part of the Hive core — it will be a plugin. The task system + comments + agent descriptions provide sufficient shared context. Core does not prescribe a backend (e.g. Obsidian, wiki, or vector DB); the plugin may use any store or integration.
...
Things Hive explicitly does **not** do:

- **Not an Agent runtime** — Hive orchestrates, Agents run elsewhere
...
1. **Unopinionated about how you run your Agents.** Any language, any framework, any runtime. Hive is the control plane, not the execution plane.
...
9. **Extensible core.** Clean boundaries so plugins can add capabilities (knowledge base, revenue tracking, etc.) without modifying core.
