...
async function ensureEmbeddedPostgresConnection(
  ...
  if (runningPid) {
    ...
    const adminConnectionString = `postgres://hive:hive@127.0.0.1:${port}/postgres`;
    await ensurePostgresDatabase(adminConnectionString, "hive");
    return {
      connectionString: `postgres://hive:hive@127.0.0.1:${port}/hive`,
  ...
  const adminConnectionString = `postgres://hive:hive@127.0.0.1:${preferredPort}/postgres`;
  await ensurePostgresDatabase(adminConnectionString, "hive");

  return {
    connectionString: `postgres://hive:hive@127.0.0.1:${preferredPort}/hive`,
