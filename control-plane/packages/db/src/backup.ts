...
function resolveHiveInstanceId(): string {
  const raw = process.env.HIVE_INSTANCE_ID?.trim() || "default";
...
function resolveDefaultConfigPath(): string {
  return path.resolve(resolveHiveHomeDir(), "instances", resolveHiveInstanceId(), "config.json");
}
...
function resolveDefaultBackupDir(): string {
  return path.resolve(resolveHiveHomeDir(), "instances", resolveHiveInstanceId(), "data", "backups");
}
