...
export type ResolvedDatabaseTarget =
  | {
      mode: "postgres";
      connectionString: string;
      source: "DATABASE_URL" | "hive-env" | "config.database.connectionString";
      configPath: string;
      envPath: string;
    }
  | {
...
function resolveHiveInstanceId(): string {
...
function resolveDefaultEmbeddedPostgresDir(): string {
  return path.resolve(resolveHiveHomeDir(), "instances", resolveHiveInstanceId(), "db");
}
...
function findConfigFileFromAncestors(startDir: string): string | null {
  ...
  while (true) {
    const candidate = path.resolve(currentDir, ".hive", CONFIG_BASENAME);
...
function resolveHiveConfigPath(): string {
  if (process.env.HIVE_CONFIG?.trim()) {
    return path.resolve(process.env.HIVE_CONFIG.trim());
  }
  return findConfigFileFromAncestors(process.cwd()) ?? resolveDefaultConfigPath();
}

function resolveHiveEnvPath(configPath: string): string {
...
export function resolveDatabaseTarget(): ResolvedDatabaseTarget {
  ...
  const envPath = resolveHiveEnvPath(configPath);
