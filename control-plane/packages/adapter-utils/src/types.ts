...
export interface AdapterExecutionResult {
  ...
  /**
   * When true, the control plane must not finalize the run in executeRun; completion
   * will be reported asynchronously (e.g. via WebSocket from a remote worker).
   */
  completionDeferred?: boolean;
  /**
   * When true, heartbeat should revert the run to `queued` and rely on placement retry
   * scheduling (managed worker placement v1); adapter execution already released resources.
   */
  requeueRun?: boolean;
  question?: {
    prompt: string;
...
export interface AdapterExecutionContext {
  ...
  authToken?: string;
  /** Optional adapter key for the worker (e.g. codex_acp). When set, sent in the run message so the worker selects that executor. */
  adapterKey?: string;
}
...
export interface CreateConfigValues {
  ...
  url: string;
  /** @deprecated Worker connects via WebSocket; no URL required. Kept for backwards compatibility. */
  workerUrl?: string;
  timeoutMs?: number;
  runPath?: string;
