...
Hive enforces a strict organizational hierarchy. Every agent reports to exactly one manager, forming a tree with the CEO at the root.

Departments are separate from the reporting tree. Agents and users can belong to multiple departments for assignment governance.
...
- **Department-scoped assignment** — when an issue has a department, assignees must be members of that department unless the actor has `tasks:assign_scope`
