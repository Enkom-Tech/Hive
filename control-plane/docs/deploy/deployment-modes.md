...
Hive supports two runtime modes with different security profiles.
...
```sh
pnpm hive allowed-hostname my-machine
```
...
When migrating from `local_trusted` to `authenticated`, Hive emits a one-time claim URL at startup:
...
```sh
HIVE_DEPLOYMENT_MODE=authenticated pnpm hive run
```
