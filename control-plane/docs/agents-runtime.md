...
For `claude_local` and `codex_local`, Hive assumes the CLI is already installed and authenticated on the host machine.
...
Hive stores session IDs for resumable adapters.
...
- If `ANTHROPIC_API_KEY` is set in adapter env or host environment, Claude uses API-key auth instead of subscription login. Hive surfaces this as a warning in environment tests, not a hard error.
