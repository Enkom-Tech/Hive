# ClipHub: Marketplace for Hive Team Configurations

> The "app store" for whole-company AI teams — pre-built Hive configurations, agent blueprints, skills, and governance templates that ship real work from day one.

## 1. Vision & Positioning

**ClipHub** sells **entire team configurations** — org charts, agent roles, inter-agent workflows, governance rules, and project templates — for Hive-managed companies.

| Dimension | ClipHub |
|---|---|
| Unit of sale | Team blueprint (multi-agent org) |
| Buyer | Founder / team lead spinning up an AI company |
| Install target | Hive company (agents, projects, governance) |
...
A complete Hive company configuration:
...
Single-agent configurations designed to plug into a Hive org:
...
Portable skill files that any Hive agent can use:

- Markdown skill files with instructions
- Tool configurations and shell scripts
- Compatible with Hive's skill loading system
...
interface Listing {
  ...
  hiveVersionMin: string;     // Minimum Hive version
...
interface Purchase {
  ...
  listingId: string;
  buyerUserId: string;
  buyerCompanyId: string | null;    // Target Hive company
...
| `POST` | `/api/listings/:id/install` | Install to Hive company |
...
Homepage → Browse marketplace → Filter by type/category
  ...
  → Select target Hive company (or create new)
  → ClipHub API calls Hive API to:
  ...
  → Redirect to Hive dashboard with new team running
```
...
### 5.3 Creator: Export from Hive → Publish

```
Running Hive company → "Export as Blueprint" (CLI or UI)
  → Hive exports:
...
- **Color palette**: Dark ink primary, warm sand backgrounds, accent color for CTAs (Hive brand blue/purple)
...
| Install flow | One-click deploy to Hive company |
...
{
  "targetCompanyId": "uuid",         // Existing Hive company
...
3. For each agent in blueprint:
   - `POST /api/companies/:id/agents` (if `hive-create-agent` supports it, or via approval flow)
...
- **Frontend**: Next.js (React), Tailwind CSS, same UI framework as Hive
- **Backend**: Node.js API (or extend Hive server)
- **Database**: Postgres (can share Hive's DB or separate)
- **Payments**: Stripe Connect (marketplace mode)
- **Storage**: S3/R2 for listing bundles and images
- **Auth**: Shared with Hive auth (or OAuth2)

### 9.2 Integration with Hive

ClipHub can be:
- **Option A**: A separate app that calls Hive's API to install blueprints
- **Option B**: A built-in section of the Hive UI (`/marketplace` route)

Option B is simpler for MVP — adds routes to the existing Hive UI and API.
...
- [ ] Install flow: blueprint → Hive company
