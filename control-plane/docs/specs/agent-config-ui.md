...
Agents are the employees of a Hive company. Each agent uses the **managed_worker** adapter: the control plane talks to the worker via a WebSocket link (worker connects to the control plane), which runs the agent. Agents have a position in the org chart (who they report to), a heartbeat policy (how/when they wake up), and a budget. The UI at `/agents` supports creating and configuring agents, viewing their org hierarchy, and inspecting run history, live logs, and accumulated costs.
...
|-------|---------|---------|-------|
| Adapter Type | Read-only | `managed_worker` | Single adapter type; worker runs agents |
| Test environment | Button | -- | Runs worker/config diagnostics (pass/warn/fail) for current unsaved config |
| Timeout (ms) | Number input | 15000 | Run timeout (1000–300000 ms). Worker connects to the control plane via WebSocket; no URL required. |
...
All endpoints already exist. No new server work needed for the initial release.
