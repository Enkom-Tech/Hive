...
Environment overrides:
  ...
  PC_DATA=/tmp/data           Hive data dir to use
  HIVE_HOST=127.0.0.1    Host passed to the onboarded server
  HIVE_PORT=3232         Port passed to the onboarded server
...
export PC_TEST_ROOT="${PC_TEST_ROOT:-$(mktemp -d /tmp/hive-clean-ref.XXXXXX)}"
export PC_HOME="${PC_HOME:-$PC_TEST_ROOT/home}"
export PC_CACHE="${PC_CACHE:-$PC_TEST_ROOT/npm-cache}"
export PC_DATA="${PC_DATA:-$PC_TEST_ROOT/hive-data}"
export PC_REPO="${PC_REPO:-$PC_TEST_ROOT/repo}"
export HIVE_HOST="${HIVE_HOST:-127.0.0.1}"
export HIVE_PORT="${HIVE_PORT:-3100}"
export HIVE_OPEN_ON_LISTEN="${HIVE_OPEN_ON_LISTEN:-false}"
...
env \
  ...
  HOST="$HIVE_HOST" \
  PORT="$HIVE_PORT" \
  HIVE_OPEN_ON_LISTEN="$HIVE_OPEN_ON_LISTEN" \
  pnpm hive onboard --yes --data-dir "$PC_DATA"
