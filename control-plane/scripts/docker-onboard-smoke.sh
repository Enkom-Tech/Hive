...
IMAGE_NAME="${IMAGE_NAME:-hive-onboard-smoke}"
HOST_PORT="${HOST_PORT:-3131}"
HIVE_VERSION="${HIVE_VERSION:-latest}"
DATA_DIR="${DATA_DIR:-$REPO_ROOT/data/docker-onboard-smoke}"
HOST_UID="${HOST_UID:-$(id -u)}"
HIVE_DEPLOYMENT_MODE="${HIVE_DEPLOYMENT_MODE:-authenticated}"
HIVE_DEPLOYMENT_EXPOSURE="${HIVE_DEPLOYMENT_EXPOSURE:-private}"
HIVE_PUBLIC_URL="${HIVE_PUBLIC_URL:-http://localhost:${HOST_PORT}}"
SMOKE_AUTO_BOOTSTRAP="${SMOKE_AUTO_BOOTSTRAP:-true}"
SMOKE_ADMIN_NAME="${SMOKE_ADMIN_NAME:-Smoke Admin}"
SMOKE_ADMIN_EMAIL="${SMOKE_ADMIN_EMAIL:-smoke-admin@hive.local}"
SMOKE_ADMIN_PASSWORD="${SMOKE_ADMIN_PASSWORD:-hive-smoke-password}"
...
generate_bootstrap_invite_url() {
  ...
  if bootstrap_output="$(
    docker exec \
      -e HIVE_DEPLOYMENT_MODE="$HIVE_DEPLOYMENT_MODE" \
      -e HIVE_DEPLOYMENT_EXPOSURE="$HIVE_DEPLOYMENT_EXPOSURE" \
      -e HIVE_PUBLIC_URL="$HIVE_PUBLIC_URL" \
      -e HIVE_HOME="/hive" \
      "$CONTAINER_NAME" bash -lc \
      'timeout 20s npx --yes "hive@${HIVE_VERSION}" auth bootstrap-ceo --data-dir "$HIVE_HOME" --base-url "$HIVE_PUBLIC_URL"' \
      2>&1
  )"; then
    bootstrap_status=0
  else
...
sign_up_or_sign_in() {
  ...
  signup_status="$(post_json_with_cookies \
    "$HIVE_PUBLIC_URL/api/auth/sign-up/email" \
  ...
  signin_status="$(post_json_with_cookies \
    "$HIVE_PUBLIC_URL/api/auth/sign-in/email" \
...
auto_bootstrap_authenticated_smoke() {
  local health_url="$HIVE_PUBLIC_URL/api/health"
  local health_json
  ...
  if [[ "$health_json" == *'"bootstrapStatus":"ready"'* ]]; then
    ...
    accept_status="$(post_json_with_cookies \
      "$HIVE_PUBLIC_URL/api/invites/$invite_token/accept" \
      '{"requestType":"human"}' \
      "$accept_response")"
  ...
  local session_json
  session_json="$(get_with_cookies "$HIVE_PUBLIC_URL/api/auth/get-session")"
  ...
  companies_json="$(get_with_cookies "$HIVE_PUBLIC_URL/api/companies")"
...
echo "    Public URL: $HIVE_PUBLIC_URL"
echo "    Smoke auto-bootstrap: $SMOKE_AUTO_BOOTSTRAP"
echo "    Data dir: $DATA_DIR"
echo "    Deployment: $HIVE_DEPLOYMENT_MODE/$HIVE_DEPLOYMENT_EXPOSURE"
...
TMP_DIR="$(mktemp -d "${TMPDIR:-/tmp}/hive-onboard-smoke.XXXXXX")"
COOKIE_JAR="$TMP_DIR/cookies.txt"

if ! wait_for_http "$HIVE_PUBLIC_URL/api/health" 90 1; then
  echo "Smoke bootstrap failed: server did not become ready at $HIVE_PUBLIC_URL/api/health" >&2
  exit 1
fi

if [[ "$SMOKE_AUTO_BOOTSTRAP" == "true" && "$HIVE_DEPLOYMENT_MODE" == "authenticated" ]]; then
  auto_bootstrap_authenticated_smoke
fi
