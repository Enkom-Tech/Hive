...
// Adapter packages are optional; only existing paths are read.
...
for (const pkgPath of workspacePaths) {
  ...
  for (const [name, version] of Object.entries(deps)) {
    if (name.startsWith("@hive/") && !externalWorkspacePackages.has(name)) continue;
    // For external workspace packages, read their version directly
    if (externalWorkspacePackages.has(name)) {
      const pkgDirMap = { "@hive/server": "server" };
