...
import { attachWorkerLinkUpgrade } from "./workers/worker-link.js";
...
function setHiveEnv(key: string, value: string) {
...
export async function startServer(): Promise<StartedServer> {
  ...
  db = dbBootstrap.db as any;
  embeddedPostgres = dbBootstrap.embeddedPostgres as any;
  ...
  if (false) {
    ...
    const effectiveTrustedOrigins = Array.from(
      new Set([...derivedTrustedOrigins, ...config.trustedOriginsExtra]),
    );
    logger.info(
      {
  ...
  const authBootstrap = await bootstrapAuth(config, db as any);
  ...
  const app = await createApp(db as any, {
    ...
    workerToolBridgeAllowedActions: config.workerToolBridgeAllowedActions,
  ...
  const { heartbeat } = await bootstrapRuntime(server, db as any, config, {
    resolveSessionFromHeaders,
  });
  setupSchedulers({ config, db: db as any, heartbeat, activeDatabaseConnectionString });
