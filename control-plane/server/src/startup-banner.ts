...
import { resolveHiveConfigPath, resolveHiveEnvPath } from "./paths.js";
...
type StartupBannerOptions = {
  ...
  /** When true, show agent JWT secret as set (from config). When false, may still show warn from env file. */
  agentJwtSecretSet: boolean;
};
...
function resolveAgentJwtSecretStatus(
  opts: { agentJwtSecretSet: boolean; envFilePath: string },
): {
  status: "pass" | "warn";
  message: string;
} {
  if (opts.agentJwtSecretSet) {
    return { status: "pass", message: "set" };
  }

  if (existsSync(opts.envFilePath)) {
    const parsed = parseEnvFileContents(readFileSync(opts.envFilePath, "utf-8"));
    const fileValue =
      (typeof parsed.HIVE_AGENT_JWT_SECRET === "string"
        ? parsed.HIVE_AGENT_JWT_SECRET
        : typeof parsed.HIVE_AGENT_JWT_SECRET === "string"
          ? parsed.HIVE_AGENT_JWT_SECRET
          : "")?.trim() ?? "";
    if (fileValue) {
      return {
        status: "warn",
        message: `found in ${opts.envFilePath} but not loaded`,
      };
    }
  }
  ...
  return {
    status: "warn",
    message: "missing (run `pnpm hive onboard`)",
  };
}
...
export function printStartupBanner(opts: StartupBannerOptions): void {
  ...
  const envFilePath = resolveHiveEnvPath();
  const agentJwtSecret = resolveAgentJwtSecretStatus({
    agentJwtSecretSet: opts.agentJwtSecretSet,
    envFilePath,
  });
