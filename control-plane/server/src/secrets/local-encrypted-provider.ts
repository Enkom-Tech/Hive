...
function resolveMasterKeyFilePath() {
  const fromEnv = process.env.HIVE_SECRETS_MASTER_KEY_FILE ?? process.env.HIVE_SECRETS_MASTER_KEY_FILE;
...
function loadOrCreateMasterKey(): Buffer {
  const envKeyRaw =
    process.env.HIVE_SECRETS_MASTER_KEY ?? process.env.HIVE_SECRETS_MASTER_KEY;
  if (envKeyRaw && envKeyRaw.trim().length > 0) {
    const fromEnv = decodeMasterKey(envKeyRaw);
    if (!fromEnv) {
      throw badRequest(
        "Invalid HIVE_SECRETS_MASTER_KEY / HIVE_SECRETS_MASTER_KEY (expected 32-byte base64, 64-char hex, or raw 32-char string)",
      );
    }
    return fromEnv;
  }
