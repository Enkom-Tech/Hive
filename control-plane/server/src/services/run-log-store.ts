...
/** Set run log base path from server config. Call once at server startup. */
export function setRunLogBasePath(basePath: string): void {
  runLogBasePath = basePath;
  cachedStore = null;
}
...
export function getRunLogStore(): RunLogStore {
  ...
  if (!runLogBasePath) {
    throw new Error("Run log base path not set. Call setRunLogBasePath() at server startup.");
  }
