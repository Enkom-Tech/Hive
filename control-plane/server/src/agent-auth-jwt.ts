...
function jwtConfig() {
  const secret = hiveEnv("AGENT_JWT_SECRET");
  if (!secret) return null;

  return {
    secret,
    ttlSeconds: parseNumber(hiveEnv("AGENT_JWT_TTL_SECONDS"), 60 * 60 * 48),
    issuer: hiveEnv("AGENT_JWT_ISSUER") ?? "hive",
    audience: hiveEnv("AGENT_JWT_AUDIENCE") ?? "hive-api",
  };
}
