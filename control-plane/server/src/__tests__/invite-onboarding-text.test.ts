...
describe("buildInviteOnboardingTextDocument", () => {
  it("renders a plain-text onboarding doc with expected endpoint references", () => {
    ...
    expect(text).toContain("Hive agent onboarding");
    ...
    expect(text).toContain("Suggested Hive base URLs to try");
    expect(text).toContain("http://localhost:3100");
    expect(text).toContain("host.docker.internal");
    expect(text).toContain("managed_worker");
    expect(text).toContain("MANAGED-WORKER-ARCHITECTURE.md");
    expect(text).toContain("set the first reachable candidate as agentDefaultsPayload.hiveApiUrl");
  });
