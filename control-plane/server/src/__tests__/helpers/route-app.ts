...
export interface FastifyRouteTestOptions {
  /**
   * A Fastify plugin (async function taking a FastifyInstance) to register.
   * By default the plugin is registered with no prefix: plugins that register
   * routes at absolute paths (e.g. `/api/companies/...`) work as-is.
   * Supply `prefix` for plugins that register relative paths, e.g.
   * `prefix: "/api/worker-api"` for `workerApiPlugin`.
   */
  plugin: (fastify: FastifyInstance) => Promise<void>;
  /**
   * Fastify route prefix applied when registering the plugin.
   * Defaults to `""` (no prefix). Set to `/api/worker-api` for worker-api routes,
   * or any other absolute prefix needed.
   */
  prefix?: string;
...
export async function createRouteTestFastify(options: FastifyRouteTestOptions): Promise<FastifyInstance> {
  const { plugin, prefix = "", principal = defaultPrincipal } = options;
