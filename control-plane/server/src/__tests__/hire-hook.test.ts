...
describe("notifyHireApproved", () => {
  it("writes success activity when adapter hook returns ok", async () => {
    ...
    expect(logActivity).toHaveBeenCalledWith(
      ...
      expect.objectContaining({
        ...
        entityId: "a1",
        details: expect.objectContaining({ source: "approval", sourceId: "ap1", adapterType: "managed_worker" }),
      }),
    );
  });
  ...
  it("does nothing when adapter has no onHireApproved", async () => {
    vi.mocked(findServerAdapter).mockReturnValue({ type: "managed_worker" } as any);
    ...
    expect(findServerAdapter).toHaveBeenCalledWith("managed_worker");
