...
describe("ui branding", () => {
  it("detects worktree mode from HIVE_IN_WORKTREE", () => {
    expect(isWorktreeUiBrandingEnabled({ HIVE_IN_WORKTREE: "true" })).toBe(true);
    expect(isWorktreeUiBrandingEnabled({ HIVE_IN_WORKTREE: "1" })).toBe(true);
    expect(isWorktreeUiBrandingEnabled({ HIVE_IN_WORKTREE: "false" })).toBe(false);
  });
  ...
  it("rewrites the favicon block for worktree instances only", () => {
    const branded = applyUiBranding(TEMPLATE, { HIVE_IN_WORKTREE: "true" });
