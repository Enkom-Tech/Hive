...
describe("deduplicateAgentName", () => {
  ...
  it("appends suffix when name collides", () => {
    const name = deduplicateAgentName("worker-agent", [
      { id: "a1", name: "worker-agent", status: "idle" },
    ]);
    expect(name).toBe("worker-agent 2");
  });

  it("increments suffix until unique", () => {
    const name = deduplicateAgentName("worker-agent", [
      { id: "a1", name: "worker-agent", status: "idle" },
      { id: "a2", name: "worker-agent-2", status: "idle" },
      { id: "a3", name: "worker-agent-3", status: "idle" },
    ]);
    expect(name).toBe("worker-agent 4");
  });

  it("ignores terminated agents for collision", () => {
    const name = deduplicateAgentName("worker-agent", [
      { id: "a1", name: "worker-agent", status: "terminated" },
    ]);
    expect(name).toBe("worker-agent");
  });
});
