...
import { hiveConfigSchema, type HiveConfig } from "./schema.js";
import {
...
function findConfigFileFromAncestors(startDir: string): string | null {
  ...
  while (true) {
    const candidate = path.resolve(currentDir, ".hive", DEFAULT_CONFIG_BASENAME);
...
export function resolveConfigPath(overridePath?: string): string {
  ...
  const configEnv = process.env.HIVE_CONFIG;
  if (configEnv) return path.resolve(configEnv);
  return findConfigFileFromAncestors(process.cwd()) ?? resolveDefaultConfigPath(resolveHiveInstanceId());
}
...
export function readConfig(configPath?: string): HiveConfig | null {
  const filePath = resolveConfigPath(configPath);
  if (!fs.existsSync(filePath)) return null;
  const raw = parseJson(filePath);
  const migrated = migrateLegacyConfig(raw);
  const parsed = hiveConfigSchema.safeParse(migrated);
  if (!parsed.success) {
