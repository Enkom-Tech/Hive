...
const JWT_SECRET_ENV_KEY = "HIVE_AGENT_JWT_SECRET";
...
export function resolveHiveEnvFile(configPath?: string): string {
...
export function loadHiveEnvFile(configPath?: string): void {
...
export function writeAgentJwtEnv(secret: string, filePath = resolveEnvFilePath()): void {
  mergeHiveEnvEntries({ [JWT_SECRET_ENV_KEY]: secret }, filePath);
}

export function readHiveEnvEntries(filePath = resolveEnvFilePath()): Record<string, string> {
  if (!fs.existsSync(filePath)) return {};
  return parseEnvFile(fs.readFileSync(filePath, "utf-8"));
}

export function writeHiveEnvEntries(entries: Record<string, string>, filePath = resolveEnvFilePath()): void {
...
export function mergeHiveEnvEntries(
  entries: Record<string, string>,
  filePath = resolveEnvFilePath(),
): Record<string, string> {
  const current = readHiveEnvEntries(filePath);
  const next = {
    ...current,
    ...Object.fromEntries(
  ...
  writeHiveEnvEntries(next, filePath);
  return next;
}
