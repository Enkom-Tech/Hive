...
import { registerWorkerCommands } from "./commands/worker-link.js";

const program = new Command();
const DATA_DIR_OPTION_HELP =
  "Hive data directory root (isolates state from ~/.hive)";

program
  .name("hive")
  .description("Hive CLI — setup, diagnose, and configure your instance")
  .version("0.2.7");
...
program.hook("preAction", (_thisCommand, actionCommand) => {
  ...
  loadHiveEnvFile(options.config);
});
...
program
  ...
  .option("--run", "Start Hive immediately after saving config", false)
  .action(onboard);

program
  .command("doctor")
  .description("Run diagnostic checks on your Hive setup")
  .option("-c, --config <path>", "Path to config file")
  .option("-d, --data-dir <path>", DATA_DIR_OPTION_HELP)
...
program
  ...
  .option("--filename-prefix <prefix>", "Backup filename prefix", "hive")
...
program
  .command("run")
  .description("Bootstrap local setup (onboard + doctor) and run Hive")
  .option("-c, --config <path>", "Path to config file")
  .option("-d, --data-dir <path>", DATA_DIR_OPTION_HELP)
...
heartbeat
  ...
  .option("--api-base <url>", "Base URL for the Hive server API")
