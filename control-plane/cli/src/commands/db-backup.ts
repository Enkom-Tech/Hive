...
import { formatDatabaseBackupResult, runDatabaseBackup } from "@hive/db";
import {
  expandHomePrefix,
...
function resolveConnectionString(configPath?: string): { value: string; source: string } {
  ...
  return {
    value: `postgres://hive:hive@127.0.0.1:${port}/hive`,
...
export async function dbBackupCommand(opts: DbBackupOptions): Promise<void> {
  printHiveCliBanner();
  p.intro(pc.bgCyan(pc.black(" hive db:backup ")));

  const configPath = resolveConfigPath(opts.config);
  const config = readConfig(opts.config);
  const connection = resolveConnectionString(opts.config);
  const defaultDir = resolveDefaultBackupDir(resolveHiveInstanceId());
  ...
  const filenamePrefix = opts.filenamePrefix?.trim() || "hive";
