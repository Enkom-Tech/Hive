...
import { ensureAgentJwtSecret, loadHiveEnvFile, mergeHiveEnvEntries, readHiveEnvEntries, resolveHiveEnvFile } from "../config/env.js";
...
function isCurrentSourceConfigPath(sourceConfigPath: string): boolean {
  const currentConfigPath = process.env.HIVE_CONFIG;
...
function resolveSourceConfigPath(opts: WorktreeInitOptions): string {
  ...
  const sourceHome = path.resolve(expandHomePrefix(opts.fromDataDir ?? "~/.hive"));
  const sourceInstanceId = sanitizeWorktreeInstanceId(opts.fromInstance ?? "default");
  return path.resolve(sourceHome, "instances", sourceInstanceId, "config.json");
}

function resolveSourceConnectionString(config: HiveConfig, envEntries: Record<string, string>, portOverride?: number): string {
  ...
  return `postgres://hive:hive@127.0.0.1:${port}/hive`;
}

export function copySeededSecretsKey(input: {
  sourceConfigPath: string;
  sourceConfig: HiveConfig;
  ...
  const sourceInlineMasterKey =
    nonEmpty(input.sourceEnvEntries.HIVE_SECRETS_MASTER_KEY) ??
    (allowProcessEnvFallback ? nonEmpty(process.env.HIVE_SECRETS_MASTER_KEY) : null);
  ...
  const sourceKeyFileOverride =
    nonEmpty(input.sourceEnvEntries.HIVE_SECRETS_MASTER_KEY_FILE) ??
    (allowProcessEnvFallback ? nonEmpty(process.env.HIVE_SECRETS_MASTER_KEY_FILE) : null);
...
async function seedWorktreeDatabase(input: {
  sourceConfigPath: string;
  sourceConfig: HiveConfig;
  targetConfig: HiveConfig;
  targetPaths: WorktreeLocalPaths;
  instanceId: string;
  seedMode: WorktreeSeedMode;
}): Promise<SeedWorktreeDatabaseResult> {
  const seedPlan = resolveWorktreeSeedPlan(input.seedMode);
  const sourceEnvFile = resolveHiveEnvFile(input.sourceConfigPath);
  const sourceEnvEntries = readHiveEnvEntries(sourceEnvFile);
  ...
  try {
    ...
    const adminConnectionString = `postgres://hive:hive@127.0.0.1:${targetHandle.port}/postgres`;
    await ensurePostgresDatabase(adminConnectionString, "hive");
    const targetConnectionString = `postgres://hive:hive@127.0.0.1:${targetHandle.port}/hive`;
...
async function runWorktreeInit(opts: WorktreeInitOptions): Promise<void> {
  ...
  const sourceEnvEntries = readHiveEnvEntries(resolveHiveEnvFile(sourceConfigPath));
  const existingAgentJwtSecret =
    nonEmpty(sourceEnvEntries.HIVE_AGENT_JWT_SECRET) ??
    nonEmpty(process.env.HIVE_AGENT_JWT_SECRET);
  mergeHiveEnvEntries(
    {
      ...buildWorktreeEnvEntries(paths),
      ...(existingAgentJwtSecret ? { HIVE_AGENT_JWT_SECRET: existingAgentJwtSecret } : {}),
    },
    paths.envPath,
  );
  ensureAgentJwtSecret(paths.configPath);
  loadHiveEnvFile(paths.configPath);
  ...
  p.outro(
    pc.green(
      `Worktree ready. Run Hive inside this repo and the CLI/server will use ${paths.instanceId} automatically.`,
    ),
  );
}
...
export async function worktreeInitCommand(opts: WorktreeInitOptions): Promise<void> {
  printHiveCliBanner();
  p.intro(pc.bgCyan(pc.black(" hive worktree init ")));
  await runWorktreeInit(opts);
}

export async function worktreeMakeCommand(nameArg: string, opts: WorktreeMakeOptions): Promise<void> {
  printHiveCliBanner();
  p.intro(pc.bgCyan(pc.black(" hive worktree:make ")));
  ...
  if (existsSync(pkgJsonPath)) {
    ...
    p.log.message(pc.dim("No package.json in worktree; skipping pnpm install."));
  }
...
export async function worktreeEnvCommand(opts: WorktreeEnvOptions): Promise<void> {
  ...
  const envPath = resolveHiveEnvFile(configPath);
  const envEntries = readHiveEnvEntries(envPath);
  const out = {
    HIVE_CONFIG: configPath,
    ...(envEntries.HIVE_HOME ? { HIVE_HOME: envEntries.HIVE_HOME } : {}),
    ...(envEntries.HIVE_INSTANCE_ID ? { HIVE_INSTANCE_ID: envEntries.HIVE_INSTANCE_ID } : {}),
    ...(envEntries.HIVE_CONTEXT ? { HIVE_CONTEXT: envEntries.HIVE_CONTEXT } : {}),
    ...envEntries,
  };
...
export function registerWorktreeCommands(program: Command): void {
  const worktree = program.command("worktree").description("Worktree-local Hive instance helpers");

  program
    .command("worktree:make")
    .description("Create ~/NAME as a git worktree, then initialize an isolated Hive instance inside it")
    ...
    .option("--from-data-dir <path>", "Source HIVE_HOME used when deriving the source config")
  ...
  worktree
    .command("env")
    .description("Print shell exports for the current worktree-local Hive instance")
