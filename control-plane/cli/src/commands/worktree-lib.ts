...
export const DEFAULT_WORKTREE_HOME = "~/.hive-worktrees";
...
export function resolveWorktreeLocalPaths(opts: {
  ...
  const repoConfigDir = path.resolve(cwd, ".hive");
  return {
    cwd,
    repoConfigDir,
...
export function buildWorktreeConfig(input: {
  ...
  return {
    ...
    server: {
      ...
      authProvider: source?.server.authProvider ?? "builtin",
    ...
    storage: {
      ...
      s3: {
        bucket: source?.storage.s3.bucket ?? "hive",
...
export function buildWorktreeEnvEntries(paths: WorktreeLocalPaths): Record<string, string> {
  return {
    HIVE_HOME: paths.homeDir,
    HIVE_INSTANCE_ID: paths.instanceId,
    HIVE_CONFIG: paths.configPath,
    HIVE_CONTEXT: paths.contextPath,
    HIVE_IN_WORKTREE: "true",
  };
}
