...
async function resolveHiveSkillsDir(): Promise<string | null> {
  for (const candidate of HIVE_SKILLS_CANDIDATES) {
...
function buildAgentEnvExports(input: {
  ...
  return [
    `export HIVE_API_URL='${escaped(input.apiBase)}'`,
    `export HIVE_COMPANY_ID='${escaped(input.companyId)}'`,
    `export HIVE_AGENT_ID='${escaped(input.agentId)}'`,
    `export HIVE_API_KEY='${escaped(input.apiKey)}'`,
    `export HIVE_COMPANY_ID='${escaped(input.companyId)}'`,
    `export HIVE_AGENT_ID='${escaped(input.agentId)}'`,
    `export HIVE_API_KEY='${escaped(input.apiKey)}'`,
  ].join("\n");
...
export function registerAgentCommands(program: Command): void {
  ...
  addCommonClientOptions(
    agent
      .command("local-cli")
      .description(
        "Create an agent API key, install local Hive skills for Codex/Claude, and print shell exports",
      )
      ...
      .option(
        "--no-install-skills",
        "Skip installing Hive skills into ~/.codex/skills and ~/.claude/skills",
      )
      .action(async (agentRef: string, opts: AgentLocalCliOptions) => {
        try {
          ...
          if (opts.installSkills !== false) {
            const skillsDir = await resolveHiveSkillsDir();
            if (!skillsDir) {
              throw new Error(
                "Could not locate local Hive skills directory. Expected ./skills in the repo checkout.",
              );
            }
