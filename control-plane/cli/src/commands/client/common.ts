...
import { ApiRequestError, HiveApiClient } from "../../client/http.js";
...
export function addCommonClientOptions(command: Command, opts?: { includeCompany?: boolean }): Command {
  command
    ...
    .option("-d, --data-dir <path>", "Hive data directory root (isolates state from ~/.hive)")
    .option("--context <path>", "Path to CLI context file")
    .option("--profile <name>", "CLI context profile name")
    .option("--api-base <url>", "Base URL for the Hive API")
...
export function resolveCommandContext(
  ...
  const companyId =
    ...
    process.env.HIVE_COMPANY_ID?.trim() ||
    profile.companyId;

  if (opts?.requireCompany && !companyId) {
    throw new Error(
      "Company ID is required. Pass --company-id, set HIVE_COMPANY_ID, or set context profile companyId via `hive context set`.",
    );
  }

  const api = new HiveApiClient({ apiBase, apiKey });
  return {
    api,
    companyId,
    profileName,
...
function inferApiBaseFromConfig(configPath?: string): string {
  const envHost = process.env.HIVE_SERVER_HOST?.trim() || "localhost";
  let port = Number(process.env.HIVE_SERVER_PORT || "");
