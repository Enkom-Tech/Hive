...
export async function configure(opts: {
  ...
  printHiveCliBanner();
  p.intro(pc.bgCyan(pc.black(" hive configure ")));
  const configPath = resolveConfigPath(opts.config);

  if (!configExists(opts.config)) {
    p.log.error("No config file found. Run `hive onboard` first.");
    p.outro("");
    return;
  }
      ...
      case "secrets":
        config.secrets = await promptSecrets(config.secrets);
        {
          ...
          if (keyResult.status === "created") {
            ...
            p.log.message(pc.dim("Skipping local key file management because HIVE_SECRETS_MASTER_KEY is set"));
          }
        }
        break;
    }
