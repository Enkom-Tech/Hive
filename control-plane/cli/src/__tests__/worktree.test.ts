...
function buildSourceConfig(): HiveConfig {
  return {
    $meta: {
      version: 1,
...
describe("worktree helpers", () => {
  ...
  it("resolves worktree:make target paths under the user home directory", () => {
    expect(resolveWorktreeMakeTargetPath("hive-pr-432")).toBe(
      path.resolve(os.homedir(), "hive-pr-432"),
    );
  });

  it("rejects worktree:make names that are not safe directory/branch names", () => {
    expect(() => resolveWorktreeMakeTargetPath("hive/pr-432")).toThrow(
  ...
  it("rewrites loopback auth URLs to the new port only", () => {
    ...
    expect(rewriteLocalUrlPort("https://hive.example", 3110)).toBe("https://hive.example");
  });

  it("builds isolated config and env paths for a worktree", () => {
    const paths = resolveWorktreeLocalPaths({
      cwd: "/tmp/hive-feature",
      homeDir: "/tmp/hive-worktrees",
    ...
    expect(config.database.embeddedPostgresDataDir).toBe(
      path.resolve("/tmp/hive-worktrees", "instances", "feature-worktree-support", "db"),
    );
    ...
    expect(config.storage.localDisk.baseDir).toBe(
      path.resolve("/tmp/hive-worktrees", "instances", "feature-worktree-support", "data", "storage"),
    );

    const env = buildWorktreeEnvEntries(paths);
    expect(env.HIVE_HOME).toBe(path.resolve("/tmp/hive-worktrees"));
    expect(env.HIVE_INSTANCE_ID).toBe("feature-worktree-support");
    expect(env.HIVE_IN_WORKTREE).toBe("true");
    expect(formatShellExports(env)).toContain("export HIVE_INSTANCE_ID='feature-worktree-support'");
  });
  ...
  it("copies the source local_encrypted secrets key into the seeded worktree instance", () => {
    const tempRoot = fs.mkdtempSync(path.join(os.tmpdir(), "hive-worktree-secrets-"));
    const originalInlineMasterKey = process.env.HIVE_SECRETS_MASTER_KEY;
    const originalKeyFile = process.env.HIVE_SECRETS_MASTER_KEY_FILE;
    try {
      delete process.env.HIVE_SECRETS_MASTER_KEY;
      delete process.env.HIVE_SECRETS_MASTER_KEY_FILE;
      ...
      if (originalInlineMasterKey === undefined) {
        ...
        process.env.HIVE_SECRETS_MASTER_KEY = originalInlineMasterKey;
      }
      if (originalKeyFile === undefined) {
        delete process.env.HIVE_SECRETS_MASTER_KEY_FILE;
      } else {
        process.env.HIVE_SECRETS_MASTER_KEY_FILE = originalKeyFile;
      }
  ...
  it("writes the source inline secrets master key into the seeded worktree instance", () => {
    ...
    try {
      ...
      copySeededSecretsKey({
        ...
        sourceConfig: buildSourceConfig(),
        sourceEnvEntries: {
          HIVE_SECRETS_MASTER_KEY: "inline-source-master-key",
        },
        targetKeyFilePath: targetKeyPath,
      });
  ...
  it("persists the current agent jwt secret into the worktree env file", async () => {
    const tempRoot = fs.mkdtempSync(path.join(os.tmpdir(), "hive-worktree-jwt-"));
    const repoRoot = path.join(tempRoot, "repo");
    const originalCwd = process.cwd();
    const originalJwtSecret = process.env.HIVE_AGENT_JWT_SECRET;

    try {
      fs.mkdirSync(repoRoot, { recursive: true });
      process.env.HIVE_AGENT_JWT_SECRET = "worktree-shared-secret";
      process.chdir(repoRoot);

      await worktreeInitCommand({
        seed: false,
        fromConfig: path.join(tempRoot, "missing", "config.json"),
        home: path.join(tempRoot, ".hive-worktrees"),
      });

      const envPath = path.join(repoRoot, ".hive", ".env");
      expect(fs.readFileSync(envPath, "utf8")).toContain("HIVE_AGENT_JWT_SECRET=worktree-shared-secret");
    } finally {
      process.chdir(originalCwd);
      if (originalJwtSecret === undefined) {
        delete process.env.HIVE_AGENT_JWT_SECRET;
      } else {
        process.env.HIVE_AGENT_JWT_SECRET = originalJwtSecret;
      }
      fs.rmSync(tempRoot, { recursive: true, force: true });
    }
  });
  ...
  it("rebinds same-repo workspace paths onto the current worktree root", () => {
    const sourceRoot = path.resolve(os.tmpdir(), "example", "hive");
    const targetRoot = path.resolve(os.tmpdir(), "example", "hive-pr-432");

    expect(
      rebindWorkspaceCwd({
        sourceRepoRoot: sourceRoot,
        targetRepoRoot: targetRoot,
        workspaceCwd: sourceRoot,
      }),
    ).toBe(targetRoot);
    ...
    expect(
      rebindWorkspaceCwd({
        sourceRepoRoot: sourceRoot,
        targetRepoRoot: targetRoot,
        workspaceCwd: path.join(sourceRoot, "packages", "db"),
      }),
    ).toBe(path.join(targetRoot, "packages", "db"));
  });

  it("does not rebind paths outside the source repo root", () => {
    const sourceRoot = path.resolve(os.tmpdir(), "example", "hive");
    const targetRoot = path.resolve(os.tmpdir(), "example", "hive-pr-432");
    const otherProject = path.resolve(os.tmpdir(), "example", "other-project");

    expect(
      rebindWorkspaceCwd({
        sourceRepoRoot: sourceRoot,
        targetRepoRoot: targetRoot,
        workspaceCwd: otherProject,
      }),
    ).toBeNull();
  });
  ...
  it(
    ...
    () => {
      const tempRoot = fs.mkdtempSync(path.join(os.tmpdir(), "hive-worktree-hooks-"));
      const repoRoot = path.join(tempRoot, "repo");
  ...
  it("creates and initializes a worktree from the top-level worktree:make command", async () => {
    const tempRoot = fs.mkdtempSync(path.join(os.tmpdir(), "hive-worktree-make-"));
    const repoRoot = path.join(tempRoot, "repo");
    const fakeHome = path.join(tempRoot, "home");
    const worktreePath = path.join(fakeHome, "hive-make-test");
    const originalCwd = process.cwd();
    ...
    try {
      ...
      await worktreeMakeCommand("hive-make-test", {
        seed: false,
        home: path.join(tempRoot, ".hive-worktrees"),
      });

      expect(fs.existsSync(path.join(worktreePath, ".git"))).toBe(true);
      expect(fs.existsSync(path.join(worktreePath, ".hive", "config.json"))).toBe(true);
      expect(fs.existsSync(path.join(worktreePath, ".hive", ".env"))).toBe(true);
    } finally {
      process.chdir(originalCwd);
