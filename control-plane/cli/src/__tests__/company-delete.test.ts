...
import type { Company } from "@hive/shared";
...
describe("resolveCompanyForDeletion", () => {
  ...
  it("resolves by ID in auto mode", () => {
    ...
    expect(result.issuePrefix).toBe("HIV");
  });

  it("resolves by prefix in auto mode", () => {
    const result = resolveCompanyForDeletion(companies, "hiv", "auto");
  ...
  it("respects explicit id mode", () => {
    expect(() => resolveCompanyForDeletion(companies, "HIV", "id")).toThrow(/No company found by ID/);
  });
...
describe("assertDeleteConfirmation", () => {
  ...
  it("requires --yes", () => {
    expect(() => assertDeleteConfirmation(company, { confirm: "HIV" })).toThrow(/requires --yes/);
  });

  it("accepts matching prefix confirmation", () => {
    expect(() => assertDeleteConfirmation(company, { yes: true, confirm: "hiv" })).not.toThrow();
  });
