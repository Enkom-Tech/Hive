...
import { isUuidLike, normalizeAgentUrlKey } from "@hive/shared";
...
export const agentsApi = {
  ...
  workerConnection: (id: string, companyId?: string) =>
    api.get<{ connected: boolean }>(agentPath(id, companyId, "/worker-connection")),
  mintWorkerEnrollmentToken: (id: string, companyId?: string, body?: { ttlSeconds?: number }) =>
    api.post<{ token: string; expiresAt: string }>(
      agentPath(id, companyId, "/link-enrollment-tokens"),
      body ?? {},
    ),
  openWorkerPairingWindow: (id: string, companyId?: string, body?: { ttlSeconds?: number }) =>
    api.post<{ expiresAt: string }>(agentPath(id, companyId, "/worker-pairing-window"), body ?? {}),
  wakeup: (
    id: string,
    data: {
