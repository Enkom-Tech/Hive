...
export function Issues() {
    ...
    (nextId: string) => {
      setSearchParams(
        (prev) => {
          ...
          if (nextId.trim()) {
            next.set("departmentId", nextId.trim());
          } else {
  ...
  const { data: issues, isLoading, error } = useQuery({
    queryKey: [...queryKeys.issues.list(selectedCompanyId!), departmentIdFromUrl] as const,
    queryFn: () =>
      issuesApi.list(selectedCompanyId!, {
        ...(departmentIdFromUrl ? { departmentId: departmentIdFromUrl } : {}),
      }),
    enabled: !!selectedCompanyId,
  });
  ...
  return (
    <IssuesList
      ...
      viewStateKey="hive:issues-view"
      issueLinkState={issueLinkState}
      initialAssignees={searchParams.get("assignee") ? [searchParams.get("assignee")!] : undefined}
      initialSearch={initialSearch}
      onSearchChange={handleSearchChange}
      departments={departments}
      departmentFilterId={departmentIdFromUrl}
