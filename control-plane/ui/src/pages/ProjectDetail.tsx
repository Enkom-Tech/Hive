...
import { PROJECT_COLORS, isUuidLike } from "@hive/shared";
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="space-y-1">
        <p className="text-xs font-medium text-muted-foreground uppercase tracking-wide">Production policies</p>
        <p className="text-xs text-muted-foreground">
          Prepended to production agent runs for this project (after company and department policies). Optional.
        </p>
        <InlineEditor
          value={project.productionPolicies ?? ""}
          onSave={(productionPolicies) =>
            onUpdate({ productionPolicies: productionPolicies.trim() || null })
          }
          as="p"
          className="text-sm text-muted-foreground border border-border rounded-md px-2.5 py-2 min-h-[80px]"
          placeholder="Project-specific rules for production agents…"
          multiline
        />
      </div>
  ...
  return (
    <div className="relative" ref={ref}>
      <button
        ...
        className="shrink-0 h-5 w-5 rounded-md cursor-pointer hover:ring-2 hover:ring-foreground/20 transition-shadow"
...
function ProjectIssuesList({ projectId, companyId }: { projectId: string; companyId: string }) {
  ...
  return (
    <IssuesList
      ...
      projectId={projectId}
      viewStateKey={`hive:project-view:${projectId}`}
