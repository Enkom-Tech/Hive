...
import { parseProjectMentionHref } from "@hive/shared";
...
function MermaidDiagramBlock({ source, darkMode }: { source: string; darkMode: boolean }) {
  ...
  useEffect(() => {
    ...
    loadMermaid()
      .then(async (mermaid) => {
        ...
        const rendered = await mermaid.render(`hive-mermaid-${renderId}`, source);
        if (!active) return;
        setSvg(rendered.svg);
      })
      .catch((err) => {
  ...
  return (
    <div className="hive-mermaid">
      {svg ? (
        <div dangerouslySetInnerHTML={{ __html: svg }} />
      ) : (
        <>
          <p className={cn("hive-mermaid-status", error && "hive-mermaid-status-error")}>
            {error ? `Unable to render Mermaid diagram: ${error}` : "Rendering Mermaid diagram..."}
          </p>
          <pre className="hive-mermaid-source">
...
export function MarkdownBody({ children, className }: MarkdownBodyProps) {
  ...
  return (
    <div
      className={cn(
        "hive-markdown prose prose-sm max-w-none wrap-break-word overflow-hidden prose-pre:whitespace-pre-wrap prose-pre:wrap-break-word prose-code:break-all",
            ...
            if (parsed) {
              ...
              return (
                <a
                  ...
                  className="hive-project-mention-chip"
