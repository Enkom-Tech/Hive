...
import type { Issue } from "@hive/shared";
import {
  ISSUE_STATUS_ORDER,
  ISSUE_STATUS_LABELS,
...
function statusLabel(status: string): string {
  return ISSUE_STATUS_LABELS[status as keyof typeof ISSUE_STATUS_LABELS] ?? status.replace(/_/g, " ").replace(/\b\w/g, (c) => c.toUpperCase());
}
...
const quickFilterPresets = [
  ...
  {
    label: "Active",
    statuses: [ISSUE_STATUS_TODO, ISSUE_STATUS_IN_PROGRESS, ISSUE_STATUS_IN_REVIEW, ISSUE_STATUS_BLOCKED],
  },
  { label: "Backlog", statuses: [ISSUE_STATUS_BACKLOG] },
  { label: "Done", statuses: [...ISSUE_STATUSES_CLOSED] },
];
...
function sortIssues(issues: Issue[], state: IssueViewState): Issue[] {
  ...
  sorted.sort((a, b) => {
    switch (state.sortField) {
      case "status":
        return dir * (ISSUE_STATUS_ORDER.indexOf(a.status) - ISSUE_STATUS_ORDER.indexOf(b.status));
      case "priority":
        return dir * (ISSUE_PRIORITY_ORDER.indexOf(a.priority) - ISSUE_PRIORITY_ORDER.indexOf(b.priority));
      case "title":
...
interface IssuesListProps {
  ...
  /** When set with onDepartmentFilterChange, restricts server list + search to this department (Issues page). */
  ...
  const { data: searchedIssues = [] } = useQuery({
    ...
    queryFn: () =>
      issuesApi.list(selectedCompanyId!, {
        ...
        projectId,
        ...(departmentFilterId ? { departmentId: departmentFilterId } : {}),
      }),
    ...
    <div className="space-y-4">
      ...
      <div className="flex items-center justify-between gap-2 sm:gap-3">
        <div className="flex min-w-0 items-center gap-2 sm:gap-3">
          ...
          {onDepartmentFilterChange && (departments?.length ?? 0) > 0 && (
            <Select
              value={
                !departmentFilterId
                  ? "__all__"
                  : departments?.some((d) => d.id === departmentFilterId)
                    ? departmentFilterId
                    : "__orphan__"
              }
              onValueChange={(v) =>
                onDepartmentFilterChange(v === "__all__" || v === "__orphan__" ? "" : v)
              }
            >
              <SelectTrigger size="sm" className="w-[min(12rem,calc(100vw-8rem))] text-xs" aria-label="Filter by department">
              ...
              <SelectContent>
                ...
                {departmentFilterId &&
                  !departments?.some((d) => d.id === departmentFilterId) && (
                    <SelectItem value="__orphan__">Invalid filter (clear)</SelectItem>
                  )}
                {departments!.map((d) => (
        ...
        <div className="flex items-center gap-0.5 sm:gap-1 shrink-0">
          ...
          <Popover>
            <PopoverTrigger asChild>
              <Button variant="ghost" size="sm" className={`text-xs ${activeFilterCount > 0 ? "text-accent" : ""}`}>
