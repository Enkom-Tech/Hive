...
export function IssueProperties({ issue, onUpdate, inline }: IssuePropertiesProps) {
  ...
  const createProject = useMutation({
    mutationFn: (data: { name: string; color: string }) =>
      projectsApi.create(companyId!, { name: data.name, color: data.color }),
    onSuccess: async (created) => {
      await queryClient.invalidateQueries({ queryKey: queryKeys.projects.list(companyId!) });
      queryClient.invalidateQueries({ queryKey: queryKeys.issues.detail(issue.id) });
      queryClient.invalidateQueries({ queryKey: queryKeys.issues.list(companyId!) });
      onUpdate({
        projectId: created.id,
        executionWorkspaceSettings:
          SHOW_EXPERIMENTAL_ISSUE_WORKTREE_UI && created.executionWorkspacePolicy?.enabled
            ? {
                mode:
                  created.executionWorkspacePolicy.defaultMode === "isolated"
                    ? "isolated"
                    : "project_primary",
              }
            : null,
  ...
  const projectContent = (
    <>
      <input
        ...
        autoFocus={!inline}
      />
      {orderedProjects.length === 0 && !projectSearch.trim() && (
      ...
      <div className="mt-2 border-t border-border pt-2 space-y-1">
        ...
        <button
          type="button"
          className="flex items-center justify-center gap-1.5 w-full px-2 py-1.5 text-xs rounded border border-border hover:bg-accent/50 disabled:opacity-50"
          disabled={!companyId || !newProjectName.trim() || createProject.isPending}
          onClick={() =>
