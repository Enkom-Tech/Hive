import type { HeartbeatRun } from "@hive/shared";
import { ISSUE_STATUS_ORDER, ISSUE_STATUS_LABELS, ISSUE_PRIORITY_ORDER } from "@hive/shared";
...
export function PriorityChart({ issues }: { issues: { priority: string; createdAt: Date }[] }) {
  ...
  return (
    <div>
        ...
        {days.map(day => {
          ...
          return (
            <div key={day} className="flex-1 h-full flex flex-col justify-end" title={`${day}: ${total} issues`}>
              {total > 0 ? (
                <div className="flex flex-col-reverse gap-px overflow-hidden" style={{ height: `${heightPct}%`, minHeight: 2 }}>
                  {ISSUE_PRIORITY_ORDER.map(p => entry[p] > 0 ? (
      ...
      <DateLabels days={days} />
      <ChartLegend items={ISSUE_PRIORITY_ORDER.map(p => ({ color: priorityColors[p], label: p.charAt(0).toUpperCase() + p.slice(1) }))} />
    </div>
  );
}
...
export function IssueStatusChart({ issues }: { issues: { status: string; createdAt: Date }[] }) {
  ...
  const statusOrder = ISSUE_STATUS_ORDER.filter(s => allStatuses.has(s));
  const maxValue = Math.max(...Array.from(grouped.values()).map(v => Object.values(v).reduce((a, b) => a + b, 0)), 1);
  ...
  return (
    <div>
      ...
      <DateLabels days={days} />
      <ChartLegend items={statusOrder.map(s => ({ color: statusColors[s] ?? "#6b7280", label: ISSUE_STATUS_LABELS[s as keyof typeof ISSUE_STATUS_LABELS] ?? s }))} />
    </div>
  );
}
