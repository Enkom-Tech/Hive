...
type HiveSprite = (typeof HIVE_SPRITES)[number];
...
function spriteSize(sprite: HiveSprite): { width: number; height: number } {
  let width = 0;
...
export function AsciiArtAnimation() {
  ...
  useEffect(() => {
    ...
    function drawStaticFrame() {
      ...
      for (let baseRow = 1; baseRow < rows - 9; baseRow += gapY) {
        ...
        for (let baseCol = startX; baseCol < cols - 10; baseCol += gapX) {
          const sprite = HIVE_SPRITES[(baseCol + baseRow) % HIVE_SPRITES.length]!;
    ...
    function spawnClip() {
      const sprite = HIVE_SPRITES[Math.floor(Math.random() * HIVE_SPRITES.length)]!;
