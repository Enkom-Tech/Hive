...
description: >
  Technical guide for creating a new Hive agent adapter. Use when building
...
# Creating a Hive Agent Adapter

An adapter bridges Hive's orchestration layer to a specific AI agent runtime (Claude Code, Codex CLI, a custom process, an HTTP endpoint, etc.). Each adapter is a self-contained package that provides implementations for **three consumers**: the server, the UI, and the CLI.
...
packages/adapters/<name>/
  src/
    ...
    cli/
      ...
      format-event.ts   # Colored terminal output for `hive run --watch`
  package.json
  tsconfig.json
```
...
## 2. Shared Types (`@hive/adapter-utils`)

All adapter interfaces live in `packages/adapter-utils/src/types.ts`. Import from `@hive/adapter-utils` (types) or `@hive/adapter-utils/server-utils` (runtime helpers).
...
interface AdapterExecutionResult {
  ...
  clearSession?: boolean;         // true = tell Hive to forget the stored session
}
...
The `agentConfigurationDoc` is read by LLM agents (including Hive agents that create other agents). Write it as **routing logic**, not marketing copy. Include concrete "use when" and "don't use when" guidance so an LLM can decide whether this adapter is appropriate for a given task.
...
1. **Read config** — extract typed values from `ctx.config` using helpers (`asString`, `asNumber`, `asBoolean`, `asStringArray`, `parseObject` from `@hive/adapter-utils/server-utils`)
2. **Build environment** — call `buildHiveEnv(agent)` then layer in `HIVE_RUN_ID`, context vars (`HIVE_TASK_ID`, `HIVE_WAKE_REASON`, `HIVE_WAKE_COMMENT_ID`, `HIVE_APPROVAL_ID`, `HIVE_APPROVAL_STATUS`, `HIVE_LINKED_ISSUE_IDS`), user env overrides, and auth token
...
| `HIVE_COMPANY_ID` | `agent.companyId` |
| `HIVE_API_URL` | Server's own URL |
| `HIVE_RUN_ID` | Current run id |
| `HIVE_TASK_ID` | `context.taskId` or `context.issueId` |
| `HIVE_WAKE_REASON` | `context.wakeReason` |
| `HIVE_WAKE_COMMENT_ID` | `context.wakeCommentId` or `context.commentId` |
| `HIVE_APPROVAL_ID` | `context.approvalId` |
| `HIVE_APPROVAL_STATUS` | `context.approvalStatus` |
| `HIVE_LINKED_ISSUE_IDS` | `context.issueIds` (comma-separated) |
| `HIVE_API_KEY` | `authToken` (if no explicit key in config) |
...
Pretty-prints stdout lines for `hive run --watch`. Use `picocolors` for coloring.
...
```ts
import { execute as myExecute, sessionCodec as mySessionCodec } from "@hive/adapter-my-agent/server";
import { agentConfigurationDoc as myDoc, models as myModels } from "@hive/adapter-my-agent";
...
const myAgentAdapter: ServerAdapterModule = {
  ...
  models: myModels,
  supportsLocalAgentJwt: true,  // true if agent can use Hive API
...
import { parseMyAgentStdoutLine } from "@hive/adapter-my-agent/ui";
import { MyAgentConfigFields } from "./config-fields";
import { buildMyAgentConfig } from "@hive/adapter-my-agent/ui";
...
```ts
import { printMyAgentStreamEvent } from "@hive/adapter-my-agent/cli";
...
- **Unknown session retry**: if resume fails with a "session not found" error, retry with a fresh session and return `clearSession: true` so Hive wipes the stale session
...
Import from `@hive/adapter-utils/server-utils`:
...
| `buildHiveEnv(agent)` | Standard `HIVE_*` env vars |
...
- Package name: `@hive/adapter-<kebab-name>`
...
- Default prompt: `"You are agent {{agent.id}} ({{agent.name}}). Continue your Hive work."`
...
### Hive Skills Injection

Hive ships shared skills (in the repo's top-level `skills/` directory) that agents need at runtime — things like the `hive` API skill and the `hive-create-agent` workflow skill. Each adapter is responsible for making these skills discoverable by its agent runtime **without polluting the agent's working directory**.

**The constraint:** never copy or symlink skills into the agent's `cwd`. The cwd is the user's project checkout — writing `.claude/skills/` or any other files into it would contaminate the repo with Hive internals, break git status, and potentially leak into commits.

**The pattern:** create a clean, isolated location for skills and tell the agent runtime to look there.

**How claude-local does it:**

1. At execution time, create a fresh tmpdir: `mkdtemp("hive-skills-")`
...
async function buildSkillsDir(): Promise<string> {
  const tmp = await fs.mkdtemp(path.join(os.tmpdir(), "hive-skills-"));
  const target = path.join(tmp, ".claude", "skills");
  await fs.mkdir(target, { recursive: true });
  const entries = await fs.readdir(HIVE_SKILLS_DIR, { withFileTypes: true });
  for (const entry of entries) {
    if (entry.isDirectory()) {
      await fs.symlink(
        path.join(HIVE_SKILLS_DIR, entry.name),
...
Codex has a global personal skills directory (`$CODEX_HOME/skills` or `~/.codex/skills`). The adapter symlinks Hive skills there if they don't already exist. This is acceptable because it's the agent tool's own config directory, not the user's project.
...
**Skills as loaded procedures, not prompt bloat.** The Hive skills (like `hive` and `hive-create-agent`) are designed as on-demand procedures: the agent sees skill metadata (name + description) in its context, but only loads the full SKILL.md content when it decides to invoke a skill. This keeps the base prompt small. When writing `agentConfigurationDoc` or prompt templates for your adapter, do not inline skill content — let the agent runtime's skill discovery do the work. The descriptions in each SKILL.md frontmatter act as routing logic: they tell the agent when to load the full skill, not what the skill contains.

**Explicit vs. fuzzy skill invocation.** For production workflows where reliability matters (e.g. an agent that must always call the Hive API to report status), use explicit instructions in the prompt template: "Use the hive skill to report your progress." Fuzzy routing (letting the model decide based on description matching) is fine for exploratory tasks but unreliable for mandatory procedures.

---

## 8. Security Considerations

Adapters sit at the boundary between Hive's orchestration layer and arbitrary agent execution. This is a high-risk surface.
...
- `HIVE_API_KEY` is injected by the server into the process environment, not the prompt
...
- Prefer minimal allowlists over open internet access. An agent that only needs to call the Hive API and GitHub should not have access to arbitrary hosts.
