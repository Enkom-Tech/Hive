...
func main() {
	...
	if err = (&controllers.HiveClusterReconciler{
	...
	if err = (&controllers.HiveCompanyReconciler{
		Client: mgr.GetClient(),
		Scheme: mgr.GetScheme(),
	}).SetupWithManager(mgr); err != nil {
	...
	if err = (&controllers.HiveWorkerPoolReconciler{
		Client: mgr.GetClient(),
		Scheme: mgr.GetScheme(),
	}).SetupWithManager(mgr); err != nil {
	...
	if err = (&controllers.HiveDocIndexerReconciler{
		Client: mgr.GetClient(),
		Scheme: mgr.GetScheme(),
	}).SetupWithManager(mgr); err != nil {
		setupLog.Error(err, "unable to create controller", "controller", "HiveDocIndexer")
		os.Exit(1)
	}
